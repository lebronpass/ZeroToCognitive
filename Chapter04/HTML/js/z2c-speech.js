...
function initPage() {
    ...
    $.when($.get('/api/speech-to-text/token')).done(
        function(_token) {
            if (_token.success != 'undefined') { stt_token = _token.success;
                console.log(methodName + ' stt_token request succeeded.'); } else { stt_token = null;
                console.log(methodName + ' token request failed with: ', _token.failed); }
        }
    );
...
function onCanplaythrough() {
    ...
    try { audio.currentTime = 0; } catch (ex) { // ignore. Firefox just freaks out here for no apparent reason.
    }
